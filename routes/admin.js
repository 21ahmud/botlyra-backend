...
router.post('/users', authenticateToken, isAdmin, async (req, res) => {
  ...
  try {
    const { email, password, name, phone = '', company = '', role = 'user', plan = 'free', isActive = true } = req.body;
    ...
    const subscriptionResult = await client.query(
      ...
      [user.id, plan, isActive ? 'active' : 'inactive']
    );
...
router.put('/users/:userId', authenticateToken, isAdmin, async (req, res) => {
  ...
  try {
    ...
    const { name, email, phone, company, role, plan, isActive } = req.body;
          ...
          await client.query(
            `UPDATE subscriptions 
             SET plan = $1, 
                 ...
                 updated_at = CURRENT_TIMESTAMP
             WHERE user_id = $3`,
            [plan, isActive !== undefined ? (isActive ? 'active' : 'inactive') : null, userId]
          );
